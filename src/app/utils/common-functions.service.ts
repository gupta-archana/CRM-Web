...
export class CommonFunctionsService {
  ...
  printLog(message: any, show?: boolean) {
    if (show == undefined || show == true) console.log(message);
  }
  ...
  getEncryptedPassword(pUnencrypted: string) {
    ...
    let encryptedPass = new Array<any>(
  ...
  getLoginCredentials() {
    ...
    let encryptedPassword = this.getEncryptedPassword(
  ...
  onMenuItemClick(item, entityModel: EntityModel) {
    ...
    } else this.showErrorSnackbar("We are working on it");
  }
  ...
  setFavoriteToSessionArray(entityId) {
    ...
    if (!favAgents) favAgents = new Array<string>();
    if (this.checkFavorite(entityId)) favAgents.push(entityId);
    else favAgents.splice(favAgents.indexOf(entityId), 1);
    sessionStorage.setItem(
      this.constants.SESSION_FAV_ARRAY,
      JSON.stringify(favAgents)
    );
  ...
  checkFavorite(entityId) {
    ...
    if (favAgents) return favAgents.indexOf(entityId) == -1;
    return true;
  }
  ...
  getAddress(item: EntityModel) {
    ...
    if (address.endsWith(",")) address = address.substr(0, address.length - 1);
    return address;
  }
  ...
  showLoadedItemTagOnHeader(
    ...
    if (!hide) {
      ...
      else tag = this.constants.NO_DATA_AVAILABLE;
    }
