...
export class AppSettingComponent
  extends BaseClass
  implements OnInit, ApiResponseCallback, OnDestroy
{
  ...
  rearrangeHomeModules() {
    this.myLocalStorage.setValue(
      this.constants.SIDE_NAV_ITEMS,
      JSON.stringify(getRearrangeItemArray(this, this.constants.HOME_MODULE))
    );
    this.commonFunctions.navigateWithoutReplaceUrl(
      this.paths.PATH_REARRANGE_DRAWER_ITEM
    );
  }

  rearrangeAgentDetailModules() {
    this.myLocalStorage.setValue(
      this.constants.AGENT_DETAIL_ITEMS,
      JSON.stringify(getRearrangeItemArray(this, this.constants.AGENT_MODULE))
    );
    this.commonFunctions.navigateWithoutReplaceUrl(
      this.paths.PATH_REARRANGE_AGENT_DETAIL_ITEM
    );
  }
  rearrangePersonDetailModules() {
    this.myLocalStorage.setValue(
      this.constants.PERSON_DETAIL_ITEMS,
      JSON.stringify(getRearrangeItemArray(this, this.constants.PERSON_MODULE))
    );
    this.commonFunctions.navigateWithoutReplaceUrl(
      this.paths.PATH_REARRANGE_PERSON_DETAIL_ITEM
    );
  }

  notficationControlClick() {
    this.myLocalStorage.setValue(
      this.constants.USER_NOTIFICATIONS_CONTROLS,
      JSON.stringify(this.configNotificationModels)
    );
    this.commonFunctions.navigateWithoutReplaceUrl(
      this.paths.PATH_NOTIFICATION_CONTROL
    );
  }
  ...
  private parseResponse(element: any) {
    if (element.configCategory == "Reorder") {
      element.configuration = JSON.parse(
        element.configuration.replace(/'/g, '"')
      );
  ...
  onHomeScreenChanged(event) {
    this.selectedHomeScreen =
      this.homeScreenArray[event.target.selectedIndex].name;
    this.selectedHomeScreenPath =
      this.homeScreenArray[event.target.selectedIndex].path;
    setBasicConfigToLocalStorage(this);
    this.commonApis.updateBasicConfig(
      this.constants.HOME_SCREEN,
      this.selectedHomeScreen
    );
  }
  onBatchSizeChanged(event) {
    ...
    setBasicConfigToLocalStorage(this);
    this.commonApis.updateBasicConfig(
      this.constants.BATCH_SIZE,
      this.selectedBatchSize
    );
  }
  onSearchInChanged(event) {
    this.selectedSearchIn = event.target.value;
    this.selectedSearchInPresenter =
      this.constants.searchEntityArrayObjectUserConfig[this.selectedSearchIn];
    setBasicConfigToLocalStorage(this);
    this.commonApis.updateBasicConfig(
      this.constants.SEARCH_FILTER,
  ...
  context.configBasicModels.forEach((element) => {
    switch (element.configType.toLowerCase()) {
      case context.constants.HOME_SCREEN:
        ...
        context.selectedHomeScreenPath =
          context.constants.sideNavItemsWithPath[element.configuration];
        break;
      case context.constants.SEARCH_FILTER:
        context.selectedSearchIn = element.configuration;
        context.selectedSearchInPresenter =
          context.constants.searchEntityArrayObjectUserConfig[
            context.selectedSearchIn
          ];
        break;
...
function setReorderConfigToVariables(context: AppSettingComponent) {
  context.configReorderModels.forEach((element) => {
    ...
    if (element.configType === "HomeModules") {
      ...
      context.myLocalStorage.setValue(
        context.constants.SIDE_NAV_ITEMS,
        JSON.stringify(context.itemsArray)
      );
    }
    if (element.configType === "AgentModules") {
      context.itemsArray = getRearrangeItemArray(context, element.configType);
      context.myLocalStorage.setValue(
        context.constants.AGENT_DETAIL_ITEMS,
        JSON.stringify(context.itemsArray)
      );
    }
    if (element.configType === "PersonModules") {
      context.itemsArray = getRearrangeItemArray(context, element.configType);
      context.myLocalStorage.setValue(
        context.constants.PERSON_DETAIL_ITEMS,
        JSON.stringify(context.itemsArray)
      );
    }
  });
  ...
  console.log(
    context.myLocalStorage.getValue(context.constants.SIDE_NAV_ITEMS)
  );
  console.log(
    context.myLocalStorage.getValue(context.constants.AGENT_DETAIL_ITEMS)
  );
  console.log(
    context.myLocalStorage.getValue(context.constants.PERSON_DETAIL_ITEMS)
  );
}
function setBasicConfigToLocalStorage(context: AppSettingComponent) {
  context.myLocalStorage.setValue(
    context.constants.SELECTED_SEARCH_IN,
    context.selectedSearchInPresenter
  );
  context.myLocalStorage.setValue(
    context.constants.NUMBER_OF_ROWS,
    context.selectedBatchSize
  );
  context.myLocalStorage.setValue(
    context.constants.SELECTED_HOME_SCREEN,
    context.selectedHomeScreenPath
  );
  context.myLocalStorage.setValue(
    context.constants.SELECTED_NEWS_FEED,
    context.selectedNewsFeed
  );
}
...
function setValueToDropdowns(context: AppSettingComponent) {
  context.appSettingForm
    .get("selectedHomeScreen")
    .setValue(context.selectedHomeScreen);
  context.appSettingForm
    .get("selectedBatchSize")
    .setValue(context.selectedBatchSize);
  context.appSettingForm
    .get("selectedNewsFeed")
    .setValue(context.selectedNewsFeed);
  context.appSettingForm
    .get("selectedSearchIn")
    .setValue(context.selectedSearchIn);
}
